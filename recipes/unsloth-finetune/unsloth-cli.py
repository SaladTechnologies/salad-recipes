...
def run(args):
    ...
    # Train model (resume if checkpoints exist)
    resume_arg = args.resume_from_checkpoint if args.resume_from_checkpoint else (max([os.path.join(args.output_dir,d) for d in os.listdir(args.output_dir) if d.startswith("checkpoint-")], key=lambda x: int(x.split("-")[-1].split("/")[-1]), default=False) if args.resume and os.path.isdir(args.output_dir) else False)
...
if __name__ == "__main__":
    ...
    training_group.add_argument("--save_strategy", type=str, default="steps",
        choices=["no", "steps", "epoch"], help="When to save checkpoints.")
    training_group.add_argument("--save_steps", type=int, default=500,
        help="Save every N steps (only if save_strategy=steps).")
    training_group.add_argument("--save_total_limit", type=int, default=None,
        help="Keep only the last N checkpoints (delete older).")
    ...
    save_group.add_argument('--output_dir', type=str, default="/opt/checkpoints", help="Output directory")
    ...
    save_group.add_argument('--save_path', type=str, default="/opt/outputs", help="Path to save the model")
