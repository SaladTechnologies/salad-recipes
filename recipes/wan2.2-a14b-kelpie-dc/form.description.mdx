...
Designed for multi-GPU DataCenter nodes (e.g., **8× A100**). The container launches **one Kelpie worker per GPU**, sharing model weights between GPUs on a single node; each worker handles one job, enabling up to **8 parallel T2V jobs** per node.
