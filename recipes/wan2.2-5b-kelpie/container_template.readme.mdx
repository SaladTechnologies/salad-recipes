# Wan 2.2 TI2V-5B (Text/Image to Video) – Kelpie Recipe

This recipe runs the Wan 2.2 5B text/image-to-video model using Kelpie and S3-compatible storage. Use the included submission helpers to add jobs into the queue.
...
- [Recipe Source](https://github.com/SaladTechnologies/salad-recipes/tree/master/recipes/wan2.2-5b-kelpie)
- <Link url={`https://github.com/SaladTechnologies/salad-recipes/issues/new?title=Wan2.2-5B%20Kelpie%20Recipe&body=Image:%20${props.container.image}`}>Report an Issue</Link>
...
> Need to scale automatically? Kelpie supports **queue-aware autoscaling**, including **scale-to-zero** when the queue is empty.
